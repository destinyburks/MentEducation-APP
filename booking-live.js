...
window.MEBooking={
  client:null,
  async init(){if(!window.supabase?.createClient)throw new Error('Supabase client failed to load');this.client=window.supabase.createClient(SUPABASE_URL,SUPABASE_KEY);return this.client;},
  ...
  daysLeft(value){if(!value)return 0;return Math.max(0,Math.ceil((new Date(value)-new Date())/86400000))},
  ...
  async loadEntitlements(id){await this.requireSession();const {data,error}=await this.client.from('no_show_rebooking_entitlements').select('*').or(`source_booking_id.eq.${id},replacement_booking_id.eq.${id}`).order('created_at',{ascending:false});if(error)throw error;return data||[]},
  async loadCompensations(id){await this.requireSession();const {data,error}=await this.client.from('mentor_no_show_compensations').select('*').or(`booking_id.eq.${id},root_booking_id.eq.${id}`).order('created_at',{ascending:false});if(error)throw error;return data||[]},
  ...
  async claimNoShowRebook(entitlementId,availabilityId,notes){await this.requireSession();const {data,error}=await this.client.rpc('claim_no_show_rebooking',{p_entitlement_id:entitlementId,p_availability_id:availabilityId,p_notes:notes||null});if(error)throw error;return data},
